...
export default function RechargeHistoryPage() {
  ...
  const fetchHistory = async () => {
    try {
      ...
      let url =
        role === "admin"
          ? "https://hs-backend-2.onrender.com/api/admin/wallet/recharges"
      ...
      // Admin returns data.history, user returns array directly
        ...
        <div className="p-6">
                ...
                {loading
                  ? Array.from({ length: rowsPerPage }).map((_, idx) => (
                      <tr key={idx} className={idx % 2 === 0 ? "bg-gray-50" : "bg-white"}>
                        {Array.from({ length: role === "admin" ? 7 : 5 }).map((_, i) => (
                  ...
                  ? currentRows.map((h, index) => (
                      <tr key={h._id} className="hover:bg-gray-50 transition duration-200">
                        ...
                        {role === "admin" && (
                          <>
                            <td className="px-3 py-3 text-gray-600 text-center">{h.user?.name || "N/A"}</td>
                            <td className="px-3 py-3 text-gray-600 text-center">{h.user?.email || "N/A"}</td>
                          </>
                        )}
                        ...
                        <td className="px-3 py-3 text-gray-600 text-center">{new Date(h.createdAt).toLocaleString()}</td>
                  ...
                  : (
                    <tr>
                      <td colSpan={role === "admin" ? 7 : 6} className="text-center py-6 text-gray-500 italic">
          ...
          {!loading && history.length > rowsPerPage && (
            <div className="flex justify-between items-center mt-4 !px-1">
              <div>
                Showing {indexOfFirstRow + 1} to {Math.min(indexOfLastRow, history.length)} of {history.length} entries
              </div>
                ...
                {(() => {
                  ...
                  return Array.from({ length: endPage - startPage + 1 }, (_, i) => startPage + i).map((page) => (
                    <button
                      ...
                      className={`!px-2 !py-1 rounded ${currentPage === page ? "!bg-blue-600 !text-white" : "!bg-[#232834]"}`}
                    >
                      {page}
                    </button>
                  ));
